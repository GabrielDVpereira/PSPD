...
#define OUTFILE "out_julia_serial.bmp"
...
void check_write_permission(int process);
void  allow_next_write(int process, int num_of_process); 
...
int main(int argc, char *argv[]) {
    ...
    check_write_permission(current_process); 
    ...
    allow_next_write(current_process, num_of_process); 
...
void check_write_permission(int current_process){ 
	int can_read = 0; 

  if(current_process == MASTER_RANK) return;

  MPI_Recv(&can_read, 1, MPI_INT, current_process - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

	if(!can_read){
		printf("permission to read denied!\n");
		exit(0);  
	}
}

void allow_next_write(int current_process, int num_of_process){
  int last_process = num_of_process - 1; 
  
  if(current_process == last_process) return;
	int allow = 1; 
  MPI_Send(&allow, 1, MPI_INT, current_process + 1, 0, MPI_COMM_WORLD);
}
