...
int main(int argc, char *argv[]) {
    ...
    int total_height = atoi(argv[1]);
    int total_num_thread = omp_get_max_threads();

    int chunk = total_height / total_num_thread;
    int altura = chunk;
    int largura = 2 * total_height;
    int area = altura*largura*3;

    write_bmp_header(output_file, largura, total_height);

    #pragma omp parallel
    {
        int current_thread = omp_get_thread_num();
        int offset_start = chunk * current_thread;
        int offset_end = offset_start + chunk;

        int local_i = 0;
        unsigned char *pixel_array = calloc(area, sizeof(unsigned char));
        unsigned char *rgb = calloc(3, sizeof(unsigned char));
        ...
        for (int i = offset_start; i < offset_end; i++)
            for (int j = 0; j < largura*3; j+=3){
                compute_julia_pixel(j/3, i, largura, total_height, 1.0, rgb);
        ...
        if(current_thread > 0) fseek(output_file, area * current_thread, SEEK_SET);
