...
void execMaster(); 
void execSlave();  
void sendEndSignToSlaves();
void sendMessageToSlave(char*, int, int); 
...
void execMaster(){
    ...
    while ((c = fgetc(file)) != EOF){
        if(n >= chunkSize){
            ...
            sendMessageToSlave(buffer, chunkSize, recieverRank);
...
void getResponse(){
    ...
    int nSlaves = nprocess - 1; 

     while(endCount < nSlaves){
...
void sendEndSignToSlaves(){
    ...
    for(int i = 1; i < nprocess; i++){
        MPI_Send(&end, 1, MPI_INT, i, TAG_1, MPI_COMM_WORLD); 
        MPI_Send(&end_c, 1, MPI_CHAR, i, TAG_2, MPI_COMM_WORLD); 
    }
}

void sendMessageToSlave(char* msg, int msgSize, int recieverRank){
    // Getting the number of processes 
    MPI_Send(&msgSize, 1, MPI_INT, recieverRank, TAG_1, MPI_COMM_WORLD); 
...
void execSlave(){
    ...
    int msgSize; 
    MPI_Recv(&msgSize, 1, MPI_INT, MASTER_RANK, TAG_1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
